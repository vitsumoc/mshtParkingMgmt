...
void MgmtPage::on_btn_r2_clicked()
{
    ...
    ui->line_r3->setEnabled(true);
    ui->btn_r3->setEnabled(true);
}

void MgmtPage::on_btn_r3_clicked()
{
    // 第三轮
    QString r3Str = ui->line_r3->text();
    bool ok;
    int r3 = r3Str.toInt(&ok);
    if (!ok) {
        QMessageBox::information(this, "提示", "请输入第三轮摇号人数");
        return;
    }
    G::R3 = r3;
    G::ROUND = 3;
    _lotteryPage->changeRound();
    ui->line_r3->setEnabled(false);
    ui->btn_r3->setEnabled(false);
}
