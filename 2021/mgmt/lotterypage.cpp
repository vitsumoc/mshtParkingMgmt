...
void LotteryPage::changeRound()
{
    ...
    ui->table_round->item(0, 2)->setText(QString::number(p3));
    ...
    if (G::ROUND == 1) {
        ...
        ui->table_round->item(1, 2)->setText(QString::number(G::R3));
        ui->table_round->item(2, 2)->setText(QString::number(100.0 * p3 / G::R3) + "%");
    }
}
...
void LotteryPage::on_btn_lottery_clicked()
{
    ...
    if (!_rolling) {
        ...
        case 3:
            if (_owner->r3 >= 0) {
                QMessageBox::information(this, "提示", "业主已参与第三轮摇号");
                ui->line_code->clear();
                return;
            }
            _owner->r3_time = QDateTime::currentDateTime();
            break;
        default:
        ...
        // 双车牌只能参与第三轮
        if (G::ROUND != 3 && !_owner->plate2.isEmpty()) {
            QMessageBox::information(this, "提示", "双车牌用户只能参加第三轮摇号");
            ui->line_code->clear();
            return;
        }
...
void LotteryPage::doRoll()
{
    ...
    if (G::ROUND == 1 || G::ROUND == 2) {
        ...
        if (G::ROUND == 2) {
            _owner->r2 = score;
            // 10000 * 车位数 + 人数
            _owner->r2_all = 10000 * freeParkings.size() + freeOwners;
        }
    ...
    if (G::ROUND == 3) {
        ...
        // 0 ~ 1 的中签率，等于0则不可能中签，大于1则必中签
        double win_rate = 1.0 * parkings.size() / G::R3;
